...
const EditBook = () => {
  ...
  const validateForm = () => {
    ...
    if (isNaN(formData.pages) || formData.pages <= 0) {
      setErrorMsg("Pages must be a positive number.");
      return false;
    }
    if (isNaN(formData.publishedDate) || formData.publishedDate <= 0) {
      setErrorMsg("Published Date must be a valid year.");
      return false;
    }
    return true;
  };
  ...
  const getSubmitBook = async (e) => {
    ...
    try {
      ...
      setErrorMsg("An error occurred while updating the book.");
